...
class TrackNet(BaseNet):
    ...
    def forward(self, input):
        ...
        for level in [2, 1, 0]:
                ...
                for itr in range(0, 1):
                    T, r, delta_norm, lamb = module.dm_levenberg_marquardt_itr(T, X_a_3d, X_a_3d_sel, f_a, sel_a_idx,
                                                                               level_K, f_b, f_b_grad,
                                                                               self.lambda_prediction, level)
    ...
    def train_feed(self, I_a, d_a, sel_a_indices, K, I_b, se3_gt, epoch):
        ...
        T = torch.eye(4).view(1, 4, 4).repeat(N, 1, 1).requires_grad_()
        init_T = T

        pred_SE3_list = []                                                                  # (num_level: low_res to high_res, num_iter_per_level)
        gt_f_pair_list = []
        gt_f_patch_pair_list = []
        lambda_weight = []
        flow_list = []
        for level in [2, 1, 0]:
            ...
            x_b_2d_gt = module.batched_x_2d_normalize(float(level_H), float(level_W), x_b_2d_gt).view(N, level_H, level_W, 2)
                                                                                             # (N, H, W, 2)
            ...
            # gt_f_pair_list.append((f_a, gt_f_wrap_b))
            # unfold
            # unfold_gt_f_wrap_b = F.unfold(gt_f_wrap_b, kernel_size=5, padding=2, stride=1).view(N, f_C, 5 * 5, level_H * level_W)
            #                                                                                  # (N, C, 5*5, num_patches)
            # unfold_f_a = F.unfold(f_a, kernel_size=1, padding=0, stride=1).view(N, f_C, 1, level_H * level_W)
            #                                                                                  # (N, C, 1, num_patches)
            # unfold_gt_f_wrap_b_sel = batched_index_select(unfold_gt_f_wrap_b, 3, sel_a_idx)
            # unfold_f_a_sel = batched_index_select(unfold_f_a, 3, sel_a_idx)
            # gt_f_patch_pair_list.append((unfold_f_a_sel, unfold_gt_f_wrap_b_sel))


            # Run iteration 3 times
            for itr in range(0, 3):
                T, r, delta_norm, lamb, flow = module.dm_levenberg_marquardt_itr(T, X_a_3d, X_a_3d_sel, f_a, sel_a_idx,
        ...
        return pred_SE3_list, gt_f_pair_list, init_T.detach(), lambda_weight, flow_list, gt_f_patch_pair_list
