...
def rand_sel_subseq_sun3d(scene_frames,
    ...
    for start_frame_idx in rand_start_frame:
        ...
        if len(sub_frames) >= frames_per_subseq_num:
            # add pairwise overlap and baseline to FrameSeqData
            for i in range(len(sub_frames.frames)):
                sub_frames.frames[i]['pairwise_overlap'] = []
                sub_frames.frames[i]['pairwise_dist'] = []
                cur_frame = sub_frames.frames[i]
                cur_Tcw = sub_frames.get_Tcw(cur_frame)
                depth_path = sub_frames.get_depth_name(cur_frame)
                depth = read_sun3d_depth(os.path.join(dataset_base_dir, depth_path))
                depth[depth < 1e-5] = 1e-5
                for j in range(len(sub_frames.frames)):
                    if i == j:
                        continue
                    other_frame = sub_frames.frames[j]
                    other_Tcw = sub_frames.get_Tcw(other_frame)
                    # print(cur_Tcw)
                    # print(other_Tcw)
                    rel_dist = rel_distance(cur_Tcw, other_Tcw)
                    overlap = cam_opt.photometric_overlap(depth, K, Ta=cur_Tcw, Tb=other_Tcw, pre_cache_x2d=pre_cache_x2d)
                    # print(rel_dist, overlap)
                    sub_frames.frames[i]['pairwise_dist'].append(rel_dist)
                    sub_frames.frames[i]['pairwise_overlap'].append(overlap)
